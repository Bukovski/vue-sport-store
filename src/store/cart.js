export default {
	...
	actions: {
		...
		initializeCart(context, store) {
			context.dispatch("loadCartData"); // load data from localStorage
			
			store.watch( // if store changed
				state => state.cart.lines, //  get lines from state
				() => context.dispatch("storeCartData"), // update data into localStorage
				{ deep: true } // for update object value
			);
		}
	}
}